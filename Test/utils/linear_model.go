...
func(L *Linear_model) Init(N int){
	L.Dimsion = N
...
func (L *Linear_model) Train( Data [][]float64, Target []float64 ){
	E := 1e-4
	N := 1000
	batch_size := 10
	eta := 0.01
	...
	for{
		...
		Para_Grad(L.Coef, &(L.Para), data, target, eta)
		Y_ := L.Predict(data)
		e = Cal_Square_Error(target, Y_)
		n++
...
func (L *Linear_model) Predict( Data [][]float64)([]float64){
	N1 := L.Dimsion
	N2 := len(Data)
...
func Para_Grad(A []float64, b *float64, data [][]float64, target []float64, eta float64){
	N1 := len(A)
	N2 := len(data)
...
func Cal_Square_Error(Y []float64, Y_ []float64)(float64){
	E := 0.0
	for k,value := range Y{
		x := Y_[k]-value
		E += x*x
